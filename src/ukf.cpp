...
void UKF::PredictMeanAndCovariance() {
    ...
    std::cout << "before weights x_" << x_ << endl;
    std::cout << "weights" << weights_ << endl;
    std::cout << "Xsig_pred_" << Xsig_pred_ << endl;
    //predicted state mean
    x_.setZero();
    for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
        x_ = x_+ weights_(i) * Xsig_pred_.col(i);
        VectorXd temp = weights_(i) * Xsig_pred_.col(i);
    }
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
    ...
    P_ = (MatrixXd::Identity(5,5) - (K_ * H_)) * P_;
    
    VectorXd nis_k = y_.transpose() * S_.inverse() * y_;
    
    nis_ladar.push_back(nis_k[0]);
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
    ...
    VectorXd nis_k = y_.transpose() * S.inverse() * y_;

    nis_radar.push_back(nis_k[0]);
}
